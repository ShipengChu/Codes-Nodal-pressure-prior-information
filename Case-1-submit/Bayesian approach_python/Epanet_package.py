from ctypes import *
dll = cdll.LoadLibrary('Bayesian.dll')
...
def enInitJacoby(numThreads=2):
    errcode=dll.ENinitJacoby(c_int(numThreads))
    return errcode

def enUpdateJacoby():
    errcode=dll.ENupdateJacoby()
    return errcode

def enGetJacoby(row,col):
    value = pointer(c_double(0))#传入指针
    errcode=dll.ENgetJacoby(c_int(row),c_int(col),value)
    value = value.contents.value
    return [errcode,value]

def enCloseJacoby():
    errcode=dll.ENcloseJacoby()
    return errcode
